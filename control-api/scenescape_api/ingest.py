...
def persist(db, topic: str, raw: bytes):
    ...
    stamp=_ts(payload)
    if "/event/" in topic:
        event=Event(scene_id=scene_id,topic=topic,observed_at=stamp,payload=payload); db.add(event); db.flush()
        ...
        return event
    obs=Observation(scene_id=scene_id,topic=topic,observed_at=stamp,payload=payload); db.add(obs); return obs

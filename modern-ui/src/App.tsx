...
const rowId = (row: Row) =>
  String(row.uid ?? row.id ?? row.uuid ?? row.sensor_id ?? "");
const rowName = (row: Row) =>
  String(
    row.name ?? row.title ?? row.sensor_id ?? row.uid ?? row.id ?? "Unnamed",
  );
...
const isImageMediaPath = (value: string) =>
...
const scene2DMapCandidates = (scene: Row) => {
  ...
  return preferred.filter(
    (value, index, values) =>
      value.startsWith("/media/") && values.indexOf(value) === index,
  );
};
...
function Map2D({
  ...
  onSelect?: (selection: NonNullable<SceneSelection>) => void;
  ...
  const points = (rows: Row[]) =>
    rows.map((row) =>
      (row.points || []).map((p: any) => xy(p).join(",")).join(" "),
    );
    ...
    <div className="map-frame">
      ...
      >
          ...
          bundle.regions.map(
            (row, i) =>
              ...
              regionPoints[i] && (
                <polygon
                  key={rowId(row) || i}
                  points={regionPoints[i]}
                  role="button"
                  tabIndex={0}
                  aria-label={selectionAriaLabel("region", rowName(row))}
                  aria-pressed={
                    selected?.kind === "region" &&
                    selected.id === (rowId(row) || String(i))
                  }
                  className={
                    selected?.kind === "region" &&
                    selected.id === (rowId(row) || String(i))
                      ? "region-shape selected-entity"
                      : "region-shape"
                  }
                  onClick={(event) => {
                    event.stopPropagation();
        ...
        {visualizeRois &&
          childRegions.map(
            (row, i) =>
              childRegionPoints[i] && (
                <polygon
                  key={`child-region-${rowId(row) || i}`}
                  points={childRegionPoints[i]}
                  className="child-region-shape"
                />
              ),
          )}
        {visualizeRois &&
          bundle.tripwires.map(
            (row, i) =>
              row.visible &&
              tripPoints[i] && (
                <polyline
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={selectionAriaLabel("tripwire", rowName(row))}
                  aria-pressed={
                    selected?.kind === "tripwire" &&
                    selected.id === (rowId(row) || String(i))
                  }
                  className={
                    selected?.kind === "tripwire" &&
                    selected.id === (rowId(row) || String(i))
                      ? "tripwire-line selected-entity"
                      : "tripwire-line"
                  }
                  onClick={(event) => {
                    event.stopPropagation();
        ...
        {childTripwires.map(
          (row, i) =>
            childTripPoints[i] && (
              <polyline
                key={`child-trip-${rowId(row) || i}`}
                points={childTripPoints[i]}
                className="child-tripwire-line"
              />
            ),
        )}
        {bundle.cameras.map((camera, i) => {
          ...
          return (
            <g
              ...
              role="button"
              tabIndex={0}
              aria-label={selectionAriaLabel("camera", rowName(camera))}
              aria-pressed={
                selected?.kind === "camera" &&
                selected.id === (rowId(camera) || String(i))
              }
              className={
                selected?.kind === "camera" &&
                selected.id === (rowId(camera) || String(i))
            ...
            >
              <circle cx={x} cy={y} r="8" className="camera-dot" />
              {showLabels && (
                <text x={x + 11} y={y - 7} className="map-label">
                  {rowName(camera)}
                </text>
              )}
            </g>
        ...
        {childSensors.map((sensor, i) => {
          ...
          const center = Array.isArray(sensor.center)
            ...
            : Array.isArray(sensor.translation) && sensor.translation[0] != null
              ...
              : sensor.x != null && sensor.y != null
                ? [sensor.x, sensor.y]
                : null;
          const polygon = Array.isArray(sensor.points)
            ? sensor.points.map((point: any) => xy(point).join(",")).join(" ")
            : "";
          const position = center
            ...
            : sensor.x != null && sensor.y != null
              ...
              : null;
          return (
            <g key={`child-sensor-${rowId(sensor) || i}`}>
              {sensor.area === "poly" && polygon && (
                <polygon points={polygon} className="child-sensor-area" />
              )}
              {sensor.area === "circle" && position && (
                <circle
                  cx={position[0]}
                  cy={position[1]}
                  r={Math.max(1, Number(sensor.radius || 0) * scale)}
                  className="child-sensor-area"
                />
              )}
              {position && (
                <>
                  ...
                  {showLabels && (
                    ...
                    >
                      {rowName(sensor)} ·{" "}
                      {String(sensor.from_child_scene || "child")}
                    </text>
                  )}
                </>
              )}
        ...
        {bundle.sensors
          .filter((sensor) => Boolean(sensor.visible))
          .map((sensor, i) => {
            const center = Array.isArray(sensor.center)
              ? sensor.center
              : Array.isArray(sensor.translation) &&
                  sensor.translation[0] != null
                ? sensor.translation.slice(0, 2)
                : null;
            const polygon = Array.isArray(sensor.points)
              ? sensor.points.map((point: any) => xy(point).join(",")).join(" ")
            ...
            return (
              <g
                ...
                role="button"
                tabIndex={0}
                aria-label={selectionAriaLabel("sensor", rowName(sensor))}
                aria-pressed={
                  selected?.kind === "sensor" &&
                  selected.id === (rowId(sensor) || String(i))
                }
                className={
                  selected?.kind === "sensor" &&
                  selected.id === (rowId(sensor) || String(i))
              ...
              >
                {sensor.area === "scene" && (
                  <rect
                    x="2"
                    y="2"
                    width={Math.max(0, size[0] - 4)}
                    height={Math.max(0, size[1] - 4)}
                    className="sensor-scene-area"
                  />
                )}
                {sensor.area === "poly" && polygon && (
                  <polygon points={polygon} className="sensor-poly-area" />
                )}
                {sensor.area === "circle" && position && (
                  <circle
                    cx={position[0]}
        ...
        {(live.objects || []).map((object: Row, i: number) => {
          ...
          const label = String(
            object.category || object.type || object.id || "object",
          );
          const activeDwells =
            object.regions && typeof object.regions === "object"
              ? Object.values(object.regions as Row)
                  .filter(
                    (value: any) => value?.entered && value?.dwell != null,
                  )
                  .map((value: any) => Number(value.dwell))
              : [];
          const dwell = activeDwells.length ? Math.max(...activeDwells) : null;
          const velocity =
            Array.isArray(object.velocity) && object.velocity.length >= 2
              ? [
                  Number(object.velocity[0] || 0),
                  Number(object.velocity[1] || 0),
                ]
              : null;
          ...
          return (
            <g
              key={String(object.id ?? i)}
              role="button"
              tabIndex={0}
              aria-label={selectionAriaLabel(
                "object",
                String(object.id ?? label),
              )}
              aria-pressed={
            ...
            >
              {showHeatmap && (
                <circle
                  ...
                  cy={y}
                  r={heatRadius}
                  className="object-heatmap"
                  style={{ opacity: heatmapOpacityValue(heatmapOpacity) }}
                />
              )}
              {showHeatmap && (
                <circle
                  cx={x}
                  cy={y}
                  r={Math.max(8, heatRadius * 0.45)}
                  className="object-heatmap-core"
                  style={{ opacity: heatmapOpacityValue(heatmapOpacity) }}
                />
              )}
              {showVelocity && velocityEnd && (
                <line
                  x1={x}
                  y1={y}
                  x2={velocityEnd[0]}
                  y2={velocityEnd[1]}
                  className="object-velocity"
                  markerEnd="url(#velocity-arrow-head)"
                />
              )}
              <circle cx={x} cy={y} r="7" className="track-dot" />
              {showLabels && (
                <text x={x + 10} y={y + 4} className="map-label">
                  {label}
                </text>
              )}
      ...
      {!mapUrl && (
        <div className="map-watermark">
          No renderable 2D scene map is available; geometry and live coordinates
          are still shown.
        </div>
      )}
    </div>
...
function SceneSensorTelemetry({ sensors }: { sensors: Row[] }) {
  ...
  const load = async () => {
    ...
    await Promise.all(
      sensors.map(async (sensor) => {
        ...
        if (!id) return;
        try {
          result[id] = await apiFetch<Row[]>(
            `/api/v2/sensors/${encodeURIComponent(id)}/telemetry?limit=12`,
          );
        } catch {
          result[id] = [];
        }
  ...
  useEffect(() => {
    void load();
  }, [sensors.map(rowId).join("|")]);
  if (!sensors.length)
    return (
      <div className="empty-state">
        <h2>No sensors configured</h2>
        <p>This scene has no singleton sensors.</p>
      </div>
    );
  return (
    <section className="panel">
      <div className="panel-title">
        <div>
          <h2>Sensors & telemetry</h2>
          <p>Latest retained native sensor observations for this scene.</p>
        </div>
      ...
      <div className="sensor-runtime-grid">
        {sensors.map((sensor) => {
          const id = rowId(sensor);
          const rows = telemetry[id] || [];
          const latest = rows[0];
          return (
            <div className="sensor-runtime-card" key={id}>
              <div>
                <b>{rowName(sensor)}</b>
                <code>{id}</code>
              </div>
              <span
                className={
                  latest ? "status-pill ok-pill" : "status-pill warning-pill"
                }
              >
                {latest ? "Telemetry retained" : "No telemetry"}
              </span>
              <dl>
                <dt>Area</dt>
                <dd>{String(sensor.area || "scene")}</dd>
                <dt>Type</dt>
                <dd>{String(sensor.singleton_type || "environmental")}</dd>
                <dt>Latest</dt>
                <dd>{latest ? String(latest.timestamp || "—") : "—"}</dd>
              </dl>
              <div className="sensor-runtime-values">
                {rows.slice(0, 5).map((row, i) => (
                  <div key={String(row.id ?? i)}>
                    <b>
                      {String(
                        row.payload?.subtype || row.payload?.type || "value",
                      )}
                    </b>
                    <span>{text(row.payload?.value ?? row.payload)}</span>
                  </div>
                ))}
                {!rows.length && (
                  <div className="table-empty">No retained values.</div>
                )}
              </div>
            </div>
          );
...
function SceneRuntime({
  ...
  const age = observed
    ? Math.max(0, (Date.now() - observed.getTime()) / 1000)
    : null;
  return (
    <div className="metric-grid compact">
      <div className="metric">
        <span>Tracking feed</span>
        <strong className={live.stale ? "small-value" : "small-value ok"}>
          {live.stale ? "Stale / unavailable" : "Live"}
        </strong>
        <small>
          {age == null
            ? "No observation retained"
            : `last sample ${age.toFixed(1)}s ago`}
        </small>
      </div>
      <div className="metric">
        <span>Scene rate</span>
        <strong>{Number(live.scene_rate || 0).toFixed(1)}</strong>
        <small>Hz reported by regulated scene feed</small>
      </div>
      <div className="metric">
        <span>Tracked objects</span>
        <strong>{(live.objects || []).length}</strong>
        <small>Current retained observation</small>
      </div>
      <div className="metric">
        <span>Configured inputs</span>
        <strong>{bundle.cameras.length + bundle.sensors.length}</strong>
        <small>
          {bundle.cameras.length} cameras · {bundle.sensors.length} sensors
        </small>
      </div>
      <div className="metric">
        <span>MQTT ingestion</span>
        <strong className="small-value">
          {String(overview?.health?.mqtt || "unknown")}
        </strong>
        <small>Server-side broker heartbeat</small>
      </div>
    </div>
  );
}
...
function CameraFeed({
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      inFlight = true;
      try {
        const next = await apiObjectUrl(
          `/api/v2/cameras/${encodeURIComponent(cameraId)}/snapshot?t=${Date.now()}`,
        );
        if (!active) {
  ...
  useEffect(() => {
    ...
    const refresh = () =>
      void apiFetch<Row>(
        `/api/v2/cameras/${encodeURIComponent(cameraId)}/telemetry`,
      )
        .then((value) => active && setTelemetry(value))
        .catch(() => {});
    refresh();
  ...
  return (
    <section className="panel camera-feed-card">
      <div className="panel-title">
        <div>
          <h2>{rowName(camera)}</h2>
          <p>{cameraId}</p>
        </div>
        <span
          className={error ? "status-pill warning-pill" : "status-pill ok-pill"}
        >
          {error ? "Unavailable" : "Live JPEG"}
        </span>
      </div>
      <div className="camera-feed-frame">
        {url ? (
          <img src={url} alt={`${rowName(camera)} live view`} />
        ) : (
      ...
      {showTelemetry && (
        <div className="camera-telemetry-strip">
          <span>
            <small>FPS</small>
            <b>
              {telemetry?.fps == null
          ...
          <span>
            ...
            <b>
              {telemetry?.detections == null
                ? "Unknown"
                : Number(telemetry.detections)}
            </b>
          </span>
          <span>
            <small>Feed</small>
            <b className={telemetry?.stale ? "warning-text" : "ok-text"}>
              {telemetry
                ? telemetry.stale
                  ? "Stale"
                  : "Receiving"
  ...
  if (!cameras.length)
    return (
      <div className="empty-state">
        ...
        <p>This scene has no migrated camera resources.</p>
      </div>
    );
  return (
...
function SceneWorkspace({
  ...
  const [destination, setDestination] = useState<SceneDestination>(
    () => destinationForLegacyTab(initialTab) ?? DEFAULT_SCENE_DESTINATION,
  );
  ...
  const [showFloor, setShowFloor] = useState(
    () => localStorage.getItem("showFloor") !== "false",
  );
  ...
  const selectMode = (mode: ScenePrimaryMode) =>
    setDestination(defaultDestinationForMode(mode));
  ...
  const loadBundle = () =>
    void apiFetch<Bundle>(`/api/v2/scenes/${id}/bundle`)
      .then(setBundle)
  ...
  useEffect(() => {
    ...
    const refreshRuntime = () =>
      void apiFetch<Row>("/api/v2/overview")
        .then((value) => active && setRuntimeOverview(value))
        .catch(() => {});
    refreshRuntime();
  ...
  useEffect(() => {
    ...
    const refresh = () =>
      void apiFetch<Row>(`/api/v2/scenes/${id}/live`)
        .then((value) => active && setLive(normalizeLiveSceneState(value)))
        .catch(() => {});
    refresh();
    const acceptLive = (value: Row) => {
      ...
      setTrails((old) => {
        ...
        for (const [index, object] of normalized.objects.entries()) {
          ...
          const point = Array.isArray(object.translation)
            ? object.translation.slice(0, 2).map(Number)
            : null;
          if (!point) continue;
    ...
    void apiJsonStream<Row>(
      `/api/v2/scenes/${id}/live/stream`,
      acceptLive,
      controller.signal,
    ).catch(() => {
      if (active && !controller.signal.aborted)
        fallbackTimer = window.setInterval(refresh, 500);
    });
    return () => {
      active = false;
      controller.abort();
    ...
    return (
      <>
        ...
        <div className="center-panel">
          Loading native scene workspace…
  ...
  const chooseSelection = (
    ...
    const selectionId =
      kind === "object" ? String(row.id ?? fallback) : rowId(row) || fallback;
    setSelection({ kind, id: selectionId, value: row });
    setInspectorCollapsed(false);
  };
  const sceneStatus = deriveSceneStatus({
    ...
    mqttState: runtimeOverview?.health?.mqtt
      ? String(runtimeOverview.health.mqtt)
      : null,
  });
  ...
  return (
    <>
      ...
      <SceneStatusHeader
        sceneName={rowName(bundle.scene)}
        sceneId={id}
        status={sceneStatus}
      />
          ...
          <button
            ...
            className={
              destination.mode === mode
                ? "scene-primary-nav-item active"
                : "scene-primary-nav-item"
            }
            onClick={() => selectMode(mode)}
            aria-current={destination.mode === mode ? "page" : undefined}
          >
            {mode}
          </button>
        ))}
      ...
      <nav
        className="scene-secondary-nav"
        aria-label={`${destination.mode} views`}
      >
        {SECONDARY_VIEWS_BY_MODE[destination.mode].map((view) => {
          ...
          return (
            <button
              ...
              className={
                destination.view === view
                  ? "scene-secondary-nav-item active"
                  : "scene-secondary-nav-item"
              }
            ...
            >
              {view}
              {routesAway && (
                <span className="scene-nav-route-mark" aria-hidden="true">
                  ↗
                </span>
              )}
            </button>
      ...
      {(tab === "Live 2D" || tab === "Live 3D") && (
        <div className="scene-view-toolbar">
          <fieldset className="scene-control-group">
            ...
            <div className="scene-control-grid">
              <label>
                ...
                />
                <span>
                  Objects<small>{availability.total} tracked</small>
                </span>
              </label>
              <label>
                <input
              ...
              <label>
                ...
                />
                <span>
                  Heatmap<small>Current · {availability.total} positions</small>
                </span>
              </label>
              <label>
                <input
                ...
                <span>
                  Velocity
                  <small>
                    {availability.velocityVectors}/{availability.total} vectors
                  </small>
                </span>
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={visualizeRois}
                  onChange={(e) => setVisualizeRois(e.target.checked)}
                />
                <span>
                  Spatial overlays<small>regions · tripwires · sensors</small>
                </span>
              </label>
            </div>
            {showHeatmap && (
              <div className="scene-layer-options">
                <label>
                  Heatmap opacity{" "}
                  <input
                    type="range"
                    ...
                    onChange={(e) =>
                      setHeatmapOpacity(
                        heatmapOpacityValue(Number(e.target.value) / 100),
                      )
                    }
                  />
            ...
            <div className="scene-control-grid">
              <label>
                ...
                <span>
                  Telemetry
                  <small>
                    {live.stale ? "feed stale" : "scene stream live"}
                  </small>
                </span>
              </label>
            </div>
          ...
          {tab === "Live 2D" && (
            <fieldset className="scene-control-group scene-renderer-controls">
              <legend>2D view</legend>
              <div className="scene-control-grid">
                <label>
                  <input
                    type="checkbox"
                  ...
                  />
                  <span>
                    Labels<small>names and object categories</small>
                  </span>
                </label>
              </div>
            </fieldset>
            ...
            <fieldset className="scene-control-group scene-renderer-controls">
              ...
              <div className="scene-control-grid">
                <label>
                  ...
                  />
                  <span>
                    Floor plane<small>ground reference</small>
                  </span>
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={projectCameraFrames}
                    onChange={(e) => setProjectCameraFrames(e.target.checked)}
                  />
                  <span>
                    Camera frames<small>project live snapshots</small>
                  </span>
                </label>
                {availability.total > 0 && (
                  <label>
                    <span>
                      Tracked object
                      <select
                        aria-label="Select tracked object for inspector"
                        value={selection?.kind === "object" ? selection.id : ""}
                        onChange={(e) => {
                      ...
                      >
                        ...
                        {(live.objects || []).map((object: Row, i: number) => (
                          <option
                            key={String(object.id ?? i)}
                            value={String(object.id ?? i)}
                          >
                            {String(object.category || object.type || "object")}{" "}
                            · {String(object.id ?? i)}
                          </option>
                        ))}
                      </select>
                    ...
                    <span>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (!e.target.value) setCameraView(false);
                        }}
                      >
                        <option value="">None</option>
                        {bundle.cameras.map((camera) => (
                          <option key={rowId(camera)} value={rowId(camera)}>
                            {rowName(camera)}
                          </option>
                        ))}
                      </select>
              ...
              <div className="scene-control-ranges">
                <label>
                  Camera opacity{" "}
                  <input
                    type="range"
                    min="0"
                    max="100"
                    value={Math.round(cameraOpacity * 100)}
                    onChange={(e) =>
                      setCameraOpacity(Number(e.target.value) / 100)
                    }
                  />
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setLightIntensity(Number(e.target.value) / 100)
                    }
                  />
          ...
          <div className="scene-view-actions">
            {showTrails && (
              <button className="text-button" onClick={() => setTrails({})}>
                Clear trails
              </button>
            )}
            <button
        ...
        <div
          className={
            selection && inspectorCollapsed
              ? "scene-monitor-layout inspector-collapsed"
              : "scene-monitor-layout"
          }
        >
          <div ref={visualRef} className="scene-workspace-visual">
              ...
              <>
                <ThreeScene
                  ...
                  onSelectObject={(objectId) => {
                    const rows = live.objects || [];
                    const index = rows.findIndex(
                      (row: Row, i: number) => String(row.id ?? i) === objectId,
                    );
                    if (index >= 0)
                      chooseSelection("object", rows[index], String(index));
                  }}
                  selectedObjectId={
                    selection?.kind === "object" ? selection.id : ""
                  }
                  showTrackedObjects={liveView}
                  showSpatial={visualizeRois}
                  showFloor={showFloor}
                  showHeatmap={showHeatmap}
                  showVelocity={showVelocity}
                  heatmapOpacity={heatmapOpacity}
                  cameras={bundle.cameras}
                  projectCameraFrames={projectCameraFrames}
                  cameraOpacity={cameraOpacity}
                  selectedCameraId={selectedCameraId}
                  useSelectedCameraView={cameraView}
                  lightIntensity={lightIntensity}
                  scale={Number(bundle.scene.scale || 100)}
                  meshTranslation={bundle.scene.mesh_translation}
                  meshRotation={bundle.scene.mesh_rotation}
                  meshScale={bundle.scene.mesh_scale}
                  regions={bundle.regions}
                  tripwires={bundle.tripwires}
            ...
            <SceneVisualizationLegend
              showHeatmap={showHeatmap}
              showVelocity={showVelocity}
              heatmapOpacity={heatmapOpacity}
              velocityVectors={availability.velocityVectors}
              objectCount={availability.total}
            />
          </div>
          <SceneInspector
            selection={selection}
      ...
      {tab === "Camera feeds" && (
        <>
          <div className="live-controls camera-feed-controls">
            <label>
              <input
                type="checkbox"
                checked={showTelemetry}
          ...
          <CameraFeeds cameras={bundle.cameras} showTelemetry={showTelemetry} />
        </>
      )}
      {tab === "Sensors & telemetry" && (
        <SceneSensorTelemetry sensors={bundle.sensors} />
      )}
      {tab === "Geometry" && (
        <SpatialEditor
          scene={bundle.scene}
      ...
      {tab === "Hierarchy" && (
        <HierarchyEditor scenes={scenes} isAdmin={isAdmin} initialParent={id} />
      )}
      ...
      {tab === "Runtime" && (
        <>
          <div className="header-actions runtime-refresh">
            <button
              className="btn"
              onClick={() =>
                void apiFetch<Row>("/api/v2/overview").then(setRuntimeOverview)
              }
            >
              Refresh runtime
            </button>
      ...
      {tab === "History & replay" && (
        <section className="panel history-panel">
          <div className="panel-title">
            <div>
              <h2>Persisted observations</h2>
              <p>Metadata replay from the native historian.</p>
            </div>
            <button
              className="btn btn-primary"
              onClick={() =>
                void apiFetch<Row[]>(`/api/v2/scenes/${id}/history`).then(
                  setHistory,
                )
              }
            >
            ...
            <>
              ...
              <div className="history-list">
                {history.slice(-12).map((row) => (
                  <div key={row.id}>
                    <b>{row.timestamp}</b>
                    <span>{(row.payload?.objects || []).length} objects</span>
                  </div>
                ))}
              </div>
            </>
          ) : (
            <div className="table-empty">No retained samples loaded yet.</div>
          )}
        </section>
      )}
      {tab === "Trends & analytics" && (
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>24-hour object trend</h2>
              <p>Calculated from retained observations, not synthetic data.</p>
            </div>
            <button
              className="btn btn-primary"
              onClick={() =>
                void apiFetch<Row[]>(`/api/v2/scenes/${id}/trends`).then(
                  setTrends,
                )
              }
            >
          ...
          <div className="table-wrap">
            ...
            {!trends.length && (
              <div className="table-empty">No trend samples loaded yet.</div>
            )}
          </div>
        </section>
      )}
...
function Inventory({
  ...
  kind: "scenes" | "cameras" | "sensors" | "regions" | "tripwires";
  label: string;
  isAdmin: boolean;
}) {
  ...
  const save = async () => {
    try {
      ...
      if (selected)
        await apiFetch(
          `/api/v2/${kind}/${rowId(selected)}?revision=${selected.revision}`,
          { method: "PUT", body: JSON.stringify(body) },
        );
      else
        await apiFetch(`/api/v2/${kind}`, {
          method: "POST",
  ...
  const remove = async () => {
    if (
      !selected ||
      !window.confirm(`Delete ${rowName(selected)} from native configuration?`)
    )
      return;
    try {
      await apiFetch(`/api/v2/${kind}/${rowId(selected)}`, {
        method: "DELETE",
      });
      setSelected(null);
      load();
  ...
  return (
    <>
      ...
      <div className="toolbar">
        ...
        />
        <span className="muted">{filtered.length} configured</span>
      </div>
      {error && <div className="error-box">{error}</div>}
      <div className="config-grid">
        <section className="panel table-wrap">
            ...
            <tbody>
              {filtered.map((row) => (
                <tr key={rowId(row) || rowName(row)}>
                  ...
                  <td>{text(row.scene ?? row.scene_id ?? row.parent)}</td>
                  <td>
                    {kind === "scenes" ? (
                      <button
                        className="text-button"
                        onClick={() => go(`scene/${rowId(row)}`)}
                      >
                        Open
                      </button>
                    ) : isAdmin ? (
                      <button className="text-button" onClick={() => open(row)}>
                        Edit
                      </button>
                    ) : null}
                  </td>
          ...
          {!filtered.length && (
            <div className="table-empty">No matching resources</div>
          )}
        </section>
        {isAdmin && (selected || editor !== "{}") && (
          <section className="panel editor-panel">
            <div className="panel-title">
              <div>
                <h2>
                  {selected ? `Edit ${rowName(selected)}` : `New ${label}`}
                </h2>
                <p>Native JSON editor with revision protection.</p>
              </div>
            </div>
            <textarea
              value={editor}
...
function Zones({ goTo }: { goTo: (path: string) => void }) {
  ...
  useEffect(() => {
    void apiFetch<Row[]>("/api/v2/regions").then(setRegions);
    void apiFetch<Row[]>("/api/v2/tripwires").then(setTripwires);
  }, []);
    ...
    <>
      ...
      <div className="metric-grid compact">
        <div className="metric">
          ...
          <small>Polygon / volumetric occupancy areas</small>
        </div>
        <div className="metric">
          <span>Tripwires</span>
          <strong>{tripwires.length}</strong>
          <small>Directional crossing boundaries</small>
        </div>
        <div className="metric">
          <span>Authoring</span>
          <strong className="small-value ok">Visual</strong>
          <small>Scene map vertex editor</small>
        </div>
      </div>
      <section className="panel table-wrap">
        <table>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row._kind}-${rowId(row)}`}>
                <td>{row._kind}</td>
                <td>
                  <b>{rowName(row)}</b>
                </td>
                <td>
                  <code>{String(row.scene || "—")}</code>
                </td>
                <td>
                  {(row.points || []).length} points ·{" "}
                  {Number(row.height || 1).toFixed(2)} m
                </td>
                <td>
                  {row.visible ? "Visible" : "Hidden"}
                  {row._kind === "Region" && row.volumetric
                    ? " · volumetric"
                    : ""}
                </td>
                <td>
                  {row.scene && (
                    <button
                      className="text-button"
                      onClick={() => goTo(`scene/${row.scene}/geometry`)}
                    >
                      Open editor
                    </button>
                  )}
        ...
        {!rows.length && (
          <div className="table-empty">No regions or tripwires configured.</div>
        )}
      </section>
    </>
  );
...
function Incidents() {
  ...
  const save = async () => {
    if (!selected) return;
    const value = await apiFetch<Row>(
      `/api/v2/incidents/${selected.id}/action`,
      { method: "POST", body: JSON.stringify({ status, note, assignee }) },
    );
          ...
          <section className="panel incident-detail">
            ...
            <div className="audit-list">
              {(selected.audit || [])
                .slice()
                .reverse()
                .map((entry: Row, i: number) => (
                  <div key={i}>
                    <b>{entry.status || entry.action}</b>
                    <span>
                      {entry.at} · {entry.by || "system"}
                    </span>
                  </div>
                ))}
            </div>
...
function SceneAnalytics({
  ...
  const [rows, setRows] = useState<Row[]>([]);
  useEffect(() => {
    if (!sceneId && scenes[0]) setSceneId(rowId(scenes[0]));
  }, [scenes]);
  const load = () =>
    sceneId &&
    void apiFetch<Row[]>(`/api/v2/scenes/${sceneId}/${mode}`).then(setRows);
  return (
    <>
      <Header
      ...
      >
        <select value={sceneId} onChange={(e) => setSceneId(e.target.value)}>
          {scenes.map((scene) => (
            <option key={rowId(scene)} value={rowId(scene)}>
              {rowName(scene)}
            </option>
          ))}
        </select>
      ...
      <section className="panel table-wrap">
          ...
          <tbody>
            {rows.map((row) =>
              mode === "history" ? (
                <tr key={row.id}>
                  <td>{row.timestamp}</td>
                  <td>{(row.payload?.objects || []).length}</td>
                  <td>{row.id}</td>
                </tr>
              ) : (
                <tr key={row.bucket}>
        ...
        {!rows.length && (
          <div className="table-empty">
            Load a scene to inspect retained native data.
          </div>
        )}
      </section>
    </>
...
function App() {
  ...
  const refresh = () => {
    setApiError("");
    void apiFetch<Overview>("/api/v2/overview")
      .then(setOverview)
      .catch((e) => setApiError(String(e)));
    void apiFetch<Row[]>("/api/v2/scenes")
      .then(setScenes)
      .catch((e) => setApiError(String(e)));
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("scenescape-theme", theme);
  }, [theme]);
  useEffect(() => {
  ...
  if (!auth.ready)
    return (
      <div className="center-state">
        ...
        <h1>Securing SceneScape</h1>
        <p>Establishing your Keycloak session…</p>
      </div>
    );
  if (!auth.authenticated)
    return (
      <div className="center-state">
        ...
        <p>Verify the Keycloak realm/client configuration.</p>
  ...
  const activeScene = sceneMatch
    ? scenes.find((scene) => rowId(scene) === sceneMatch[1])
    : undefined;
  let page: ReactNode;
  if (sceneMatch && activeScene)
    page = (
      <SceneWorkspace
        scene={activeScene}
        scenes={scenes}
        ...
        initialTab={initialLegacyTabForSceneSuffix(sceneMatch[2])}
      />
    );
  else if (path === "live")
    page = (
      <>
        ...
        <div className="card-grid">
          {scenes.map((scene) => (
            <section className="panel scene-card" key={rowId(scene)}>
              ...
              <h2>{rowName(scene)}</h2>
              <code>{rowId(scene)}</code>
              <div className="scene-meta">
                <span>{scene.map ? "Map configured" : "No map"}</span>
                <span>
                  {scene.scale ? `${scene.scale} px/m` : "Scale unknown"}
                </span>
              </div>
              <button
                className="btn btn-primary full"
                onClick={() => go(`scene/${rowId(scene)}`)}
              >
                Open native 2D / 3D scene
              </button>
            </section>
          ))}
        </div>
        {!scenes.length && (
          <div className="empty-state">
            <h2>No native scenes yet</h2>
            <p>
              Run <code>./scenescape.sh recover-legacy-data</code> to copy
              existing Django configuration, or{" "}
              <code>./scenescape.sh seed-native-data</code> for the upstream
              Retail sample.
            </p>
          </div>
        )}
    ...
    page = <SceneAnalytics scenes={scenes} mode="history" />;
  else if (path === "trends")
    page = <SceneAnalytics scenes={scenes} mode="trends" />;
  else if (path === "health")
    page = (
      <>
        ...
        <div className="metric-grid compact">
          <div className="metric">
            ...
            <strong className="small-value ok">Connected</strong>
            <small>FastAPI /api/v2</small>
          </div>
          <div className="metric">
            <span>Database</span>
            <strong className="small-value ok">
              {overview?.health.database || "Unknown"}
            </strong>
            <small>PostgreSQL/native tables</small>
          </div>
          <div className="metric">
            <span>MQTT historian</span>
            <strong className="small-value">
              {overview?.health.mqtt || "Unknown"}
            </strong>
            <small>
              Last observation: {overview?.health.last_observation || "none"}
            </small>
          </div>
        </div>
      </>
    ...
    page = (
      <>
        <Header
          ...
          title="Identity, access & native migration"
        />
        {auth.isAdmin ? (
          <SecurityAdmin scenes={scenes} />
        ) : (
          <section className="panel settings-list">
            ...
            <div>
              ...
              <span>
                {auth.roles
                  .filter((role) => role.startsWith("scenescape-"))
                  .join(", ") || "authenticated"}
              </span>
            </div>
            <div>
              <b>Access administration</b>
              <span>Administrator role is required.</span>
            </div>
          </section>
        )}
        <section className="panel settings-list admin-runtime">
          <div>
            <b>Browser backend</b>
            <span>
              FastAPI /api/v2/* with Keycloak OIDC; no Django identity store.
            </span>
          </div>
          <div>
            <b>Recover existing data</b>
            <code>./scenescape.sh recover-legacy-data</code>
          </div>
          <div>
            <b>Reconcile identity</b>
            <code>./scenescape.sh reconcile-identity</code>
          </div>
          <div>
            <b>Load upstream sample</b>
            <code>./scenescape.sh seed-native-data</code>
          </div>
        </section>
      </>
    );
    ...
    page = (
      <>
        ...
        <div className="metric-grid">
          <button className="metric actionable" onClick={() => go("live")}>
            <span>Active scenes</span>
            <strong>{overview?.counts.scenes ?? scenes.length}</strong>
            <small>Open native scene workspace →</small>
          </button>
          <button className="metric actionable" onClick={() => go("cameras")}>
            <span>Camera inputs</span>
            <strong>{overview?.counts.cameras ?? 0}</strong>
            <small>Inspect native configuration →</small>
          </button>
          <button className="metric actionable" onClick={() => go("zones")}>
            <span>Spatial rules</span>
            <strong>
              {(overview?.counts.regions ?? 0) +
                (overview?.counts.tripwires ?? 0)}
            </strong>
            <small>Regions + tripwires →</small>
          </button>
          <button className="metric actionable" onClick={() => go("incidents")}>
            <span>Incidents</span>
            <strong>{overview?.counts.incidents ?? 0}</strong>
            <small>Durable operator workflow →</small>
          </button>
        </div>
        <div className="workspace-grid">
          <section className="panel scene-panel">
            <div className="panel-title">
              <div>
                <h2>Native live scene workspace</h2>
                <p>
                  2D maps, live objects, geometry, calibration and WebGL 3D
                  without Django navigation.
                </p>
              </div>
            ...
            <div className="scene-canvas">
              ...
              <span className="track track-a" />
              <span className="track track-b" />
              <span className="tripwire-shape" />
              <div className="coverage-note">
                {scenes.length
                  ? `${scenes.length} scene(s) available in native configuration`
                  : "No native scene data yet — recover or seed it from Administration"}
              </div>
            </div>
          </section>
          <section className="panel incident-panel">
            <div className="panel-title">
              <div>
                ...
                <p>Current native runtime state.</p>
              </div>
            </div>
            <div className="attention-card">
              ...
              <span>
                {overview?.health.mqtt || "Unknown"} ·{" "}
                {overview?.counts.observations ?? 0} retained observations
              </span>
            </div>
            <div className="attention-card">
              <b>Keycloak session</b>
              <span>
                {auth.displayName} ·{" "}
            ...
            <div className="attention-card">
              <b>Native configuration</b>
              <span>
                {scenes.length
                  ? "Scene data available"
                  : "Recover existing legacy data or load the Retail sample"}
              </span>
            </div>
            <button
              className="btn btn-primary full"
        ...
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Configured scenes</h2>
              <p>All actions remain inside the React/FastAPI application.</p>
            </div>
          </div>
          <div className="scene-list">
            {scenes.slice(0, 8).map((scene) => (
              <div className="scene-row" key={rowId(scene)}>
                <div>
                  <b>{rowName(scene)}</b>
                  <span>{rowId(scene)}</span>
                </div>
                <button
                  className="btn"
                  onClick={() => go(`scene/${rowId(scene)}`)}
            ...
            {!scenes.length && (
              <div className="table-empty">
                No scenes imported into native tables yet.
              </div>
            )}
          </div>
        </section>
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        {operations.map(([key, label]) => (
          <button
            key={key}
            className={path === key ? "nav-item active" : "nav-item"}
            onClick={() => go(key)}
          >
            {label}
          </button>
        ...
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <div className="identity">
              ...
              <span>{auth.isAdmin ? "Administrator" : "Operator"}</span>
            </div>

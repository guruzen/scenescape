...
export const SECONDARY_VIEWS_BY_MODE: Record<
  ScenePrimaryMode,
  readonly string[]
> = {
...
export const SCENE_DESTINATION_BY_LEGACY_TAB: Record<
  LegacySceneTab,
  SceneDestination
> = {
...
export function destinationForLegacyTab(tab: string): SceneDestination | null {
  return (
    (SCENE_DESTINATION_BY_LEGACY_TAB as Record<string, SceneDestination>)[
      tab
    ] ?? null
  );
}
...
export function legacyTabForDestination(
  destination: SceneDestination,
): LegacySceneTab | null {
  const key = destinationKey(destination);
  const match = LEGACY_SCENE_TABS.find(
    (tab) => destinationKey(SCENE_DESTINATION_BY_LEGACY_TAB[tab]) === key,
  );
  return match ?? null;
}

export function defaultDestinationForMode(
  mode: ScenePrimaryMode,
): SceneDestination {
...
export function initialLegacyTabForSceneSuffix(

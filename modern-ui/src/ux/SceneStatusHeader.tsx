...
export default function SceneStatusHeader({
  ...
  const stateClass = status.state === "LIVE"
    ? "live"
    : status.state === "DEGRADED"
      ? "degraded"
      : "stale"
  const cameraHealth = status.cameraHealthy === null
    ? `Unknown / ${status.cameraTotal}`
    : `${status.cameraHealthy} / ${status.cameraTotal}`
  const sceneRate = status.sceneRate === null ? "Unknown" : `${status.sceneRate.toFixed(1)} Hz`
  const age = status.ageSeconds === null ? "No retained observation" : `${status.ageSeconds.toFixed(1)}s ago`
  ...
  return <section className="scene-status-header" aria-label="Scene operational status">
    ...
    <div className="scene-status-metrics">
      ...
      <div><span>Scene rate</span><b>{sceneRate}</b></div>
      <div><span>Cameras</span><b>{cameraHealth}</b></div>
      <div><span>MQTT</span><b>{status.mqtt}</b></div>
      <div><span>Latest observation</span><b>{age}</b></div>
    </div>
  </section>
}

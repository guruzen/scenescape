...
export default function SceneTelemetryHud({ live }: { live: Row }) {
  ...
  return <aside className="scene-telemetry-hud" aria-label="Live scene telemetry" aria-live={LIVE_REGION_POLICY.telemetryMetrics}>
    ...
    <div className="scene-telemetry-row"><span>Scene rate</span><strong>{sceneRate}</strong></div>
    <div className="scene-telemetry-row"><span>Objects</span><strong>{telemetry.objectCount}</strong></div>
    <div className="scene-telemetry-row"><span>Observation age</span><strong>{age}</strong></div>
    <div className="scene-telemetry-camera-list">
      <span>Camera FPS</span>
      {telemetry.cameraRates.length
        ? telemetry.cameraRates.map(({ camera, fps }) => <div key={camera}><code>{camera}</code><strong>{fps.toFixed(1)}</strong></div>)
        : <em>Unknown</em>}
    </div>
  </aside>
}

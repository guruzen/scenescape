...
export function deriveSceneStatus(input: SceneStatusInput): SceneStatusModel {
  ...
  const cameraRates =
    input.cameraRates && typeof input.cameraRates === "object"
      ? input.cameraRates
      : null;
  const rateValues = cameraRates
    ? Object.values(cameraRates)
        .map(finiteNumber)
        .filter((value): value is number => value !== null)
    : [];
  const cameraHealthy = cameraRates
    ? rateValues.filter((value) => value > 0).length
    : null;
  const mqtt = String(input.mqttState || "Unknown");
  const normalizedMqtt = mqtt.toLowerCase();
  const explicitMqttFailure = !["unknown", "connected"].includes(
    normalizedMqtt,
  );
  const partialCameraHealth =
    cameraHealthy !== null && cameraTotal > 0 && cameraHealthy < cameraTotal;

...
const loadScript = (id: string, src: string) =>
  new Promise<void>((resolve, reject) => {
    ...
    script.onerror = () =>
      reject(new Error("Unable to load geospatial map provider"));
...
const canvasFile = (canvas: HTMLCanvasElement) =>
  new Promise<File>((resolve, reject) => {
    canvas.toBlob(
      (blob) =>
        blob
          ? resolve(
              new File([blob], `geospatial_map_${Date.now()}.png`, {
                type: "image/png",
              }),
            )
          : reject(new Error("Unable to generate geospatial PNG")),
      "image/png",
    );
  });
      ...
      try {
        if (provider === "google") {
          ...
          mapboxMap.addControl(
            new w.mapboxgl.NavigationControl({
              showCompass: true,
              showZoom: true,
            }),
          );
    ...
    try {
      ...
      if (provider === "google") {
        ...
        const response = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(value)}.json?access_token=${encodeURIComponent(runtimeConfig.mapboxApiKey)}`,
        );
    ...
    try {
      ...
      if (provider === "google") {
        ...
        images.forEach((image, index) =>
          context.drawImage(
            image,
            Number(centers[index][2]),
            Number(centers[index][3]),
            640,
            640,
          ),
        );
      ...
      onGenerated({
        ...
        file,
        corners,
        scale: calculateScale(current.lat, current.zoom),
      });
    } catch (e) {
      setError(String(e));
    }

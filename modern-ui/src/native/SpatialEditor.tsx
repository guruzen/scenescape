  ...
  const persisted = (row: Row, shape: SpatialKind) => {
    ...
    return shape === "region" ? (
      <polygon
        key={idOf(row)}
        points={value}
        className="spatial-existing-region"
      />
    ) : (
      <polyline
        key={idOf(row)}
        points={value}
        className="spatial-existing-tripwire"
      />
    );
  };
    ...
    <div className="spatial-map-frame">
      ...
      >
        <defs>
          ...
          >
            <path d="M0,0 L0,6 L9,3 z" className="trip-arrow-head" />
          </marker>
        </defs>
        {url && (
          <image
        ...
        {kind === "region" && draftPoints && (
          <polygon points={draftPoints} className="spatial-draft-region" />
        )}
        {kind === "tripwire" && draftPoints && (
          <polyline
            points={draftPoints}
            className="spatial-draft-tripwire"
            markerEnd="url(#trip-arrow)"
          />
        )}
        {points.map((point, index) => {
          ...
          return (
            <g key={index}>
              ...
              />
              <text x={x + 11} y={y - 9} className="map-label">
                {index + 1}
              </text>
            </g>
          );
      ...
      {!url && (
        <div className="map-watermark">
          No renderable 2D map is available; coordinates can still be authored
          on the scene canvas.
        </div>
      )}
    </div>
...
export default function SpatialEditor({
  ...
  const open = (nextKind: SpatialKind, row: Row | null) => {
    ...
    setPoints(
      Array.isArray(row?.points)
        ? row!.points.map((p: any) => [Number(p[0]), Number(p[1])])
        : [],
    );
    setRanges(
      row?.color_ranges
        ? JSON.parse(JSON.stringify(row.color_ranges))
        : JSON.parse(JSON.stringify(defaults)),
    );
    setMessage("");
    setError("");
  ...
  useEffect(() => {
    if (selectedId && !rows.some((row) => idOf(row) === selectedId))
      open(kind, null);
  }, [rows.length]);
  ...
  const save = async () => {
    ...
    if (!valid) {
      setError(
        kind === "region"
          ? "A region requires at least 3 points."
          : "A tripwire requires at least 2 points.",
      );
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      if (selected) {
        saved = await apiFetch<Row>(
          `/api/v2/${plural}/${encodeURIComponent(idOf(selected))}?revision=${selected.revision}`,
          { method: "PUT", body: JSON.stringify(payload()) },
        );
      } else {
        saved = await apiFetch<Row>(`/api/v2/${plural}`, {
          method: "POST",
          body: JSON.stringify(payload()),
        });
      }
      setSelectedId(idOf(saved));
      setDraft({ ...saved });
      setPoints(
        (saved.points || []).map((p: any) => [Number(p[0]), Number(p[1])]),
      );
      if (saved.color_ranges)
        setRanges(JSON.parse(JSON.stringify(saved.color_ranges)));
  ...
  return (
    <div className="spatial-workspace">
      <section className="panel spatial-list-panel">
        <div className="panel-title">
          <div>
            ...
            <p>Regions, volumetric occupancy and directional tripwires</p>
          </div>
        </div>
        <div className="spatial-kind-tabs">
          <button
            className={kind === "region" ? "btn active-tab" : "btn"}
            onClick={() => open("region", null)}
          >
            Regions ({regions.length})
          </button>
          <button
            className={kind === "tripwire" ? "btn active-tab" : "btn"}
            onClick={() => open("tripwire", null)}
          >
        ...
        <div className="scene-list">
          ...
          {!rows.length && (
            <div className="table-empty">
              No {kind === "region" ? "regions" : "tripwires"} configured
            </div>
          )}
        </div>
        {isAdmin && (
          <button
            className="btn btn-primary spatial-new"
            onClick={() => open(kind, null)}
          >
            New {kind}
          </button>
        )}
      ...
      <section className="panel spatial-editor-panel">
        <div className="panel-title">
          <div>
            <h2>{selected ? `Edit ${nameOf(selected)}` : `New ${kind}`}</h2>
            <p>
              Point order is preserved by the controller. Tripwire order
              determines crossing direction.
            </p>
          </div>
          {selected && <span className="status-pill">{idOf(selected)}</span>}
        </div>
        ...
        <div className="spatial-editor-grid">
          <div className="spatial-settings">
                ...
                <>
                  <label>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      value={Number(draft.buffer_size ?? 0)}
                      onChange={(e) =>
                        field("buffer_size", Number(e.target.value))
                      }
                    />
                  </label>
                  <label className="checkbox-label">
                    <input
                      type="checkbox"
                      checked={Boolean(draft.volumetric)}
                      onChange={(e) => field("volumetric", e.target.checked)}
                    />
            ...
            {kind === "region" && (
              <div className="scene-subsection">
                <h3>Occupancy color thresholds</h3>
                <div className="scene-form-grid">
            ...
            <div className="scene-subsection">
              <h3>Vertices</h3>
              <div className="spatial-point-list">
                {points.map((point, index) => (
                  <div key={index}>
                    <b>{index + 1}</b>
                    <input
                      ...
                      onChange={(e) =>
                        setPoints((old) =>
                          old.map((p, i) =>
                            i === index ? [Number(e.target.value), p[1]] : p,
                          ),
                        )
                      }
                    />
                    <input
                      ...
                      onChange={(e) =>
                        setPoints((old) =>
                          old.map((p, i) =>
                            i === index ? [p[0], Number(e.target.value)] : p,
                          ),
                        )
                      }
                    />
              ...
              <div className="editor-actions">
                ...
                {kind === "tripwire" && points.length >= 2 && (
                  <button
                    className="btn"
                    onClick={() => setPoints((old) => [...old].reverse())}
                  >
                    Reverse direction
                  </button>
                )}
          ...
          <div className="spatial-map-column">
            <div className="sensor-map-help">
              Click to add vertices. Drag an orange vertex to reposition it.
            </div>
            <GeometryMap
              scene={scene}
              regions={regions}

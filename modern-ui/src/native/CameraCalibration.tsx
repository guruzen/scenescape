...
function decodeTransforms(camera: Row): {
  cameraPoints: number[][];
  scenePoints: number[][];
} {
  if (
    camera.transform_type !== "3d-2d point correspondence" ||
    !Array.isArray(camera.transforms)
  ) {
  ...
  if (values.length >= 20 && values.length % 5 === 0) {
    ...
    const split = pairs * 2;
    const cameraPoints = Array.from({ length: pairs }, (_, i) =>
      values.slice(i * 2, i * 2 + 2),
    );
    const scenePoints = Array.from({ length: pairs }, (_, i) =>
      values.slice(split + i * 3, split + i * 3 + 3),
    );
    return { cameraPoints, scenePoints };
  }
  if (values.length >= 16 && values.length % 4 === 0) {
    ...
    const scenePoints = Array.from({ length: pairs }, (_, i) => [
      ...values.slice(split + i * 2, split + i * 2 + 2),
      0,
    ]);
    return { cameraPoints, scenePoints };
  }
...
export default function CameraCalibration({
  ...
  const camera = useMemo(
    () => cameras.find((item) => idOf(item) === cameraId) || cameras[0] || null,
    [cameras, cameraId],
  );
  ...
  useEffect(() => {
    ...
    const resolution = Array.isArray(camera.resolution)
      ? camera.resolution
      : [640, 480];
    ...
    void apiFetch<Row[]>("/api/v2/markers")
      .then((rows) =>
        setMarkers(
          rows.filter(
            (item) =>
              String(item.scene || "") === String(scene.uid || scene.id || ""),
          ),
        ),
      )
      .catch(() => setMarkers([]));
  ...
  const refreshFrame = async () => {
    ...
    try {
      const value = await apiFetch<Row>(
        `/api/v2/cameras/${encodeURIComponent(idOf(camera))}/calibration-frame?t=${Date.now()}`,
      );
      setFrame(value);
      ...
      if (Array.isArray(runtimeIntrinsics)) {
        ...
        if (flat.length >= 9)
          setIntrinsics({ fx: flat[0], fy: flat[4], cx: flat[2], cy: flat[5] });
  ...
  const calculate = async () => {
    if (pairCount < 6) {
      setError(
        "At least 6 matching point pairs are required to estimate camera intrinsics.",
      );
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      setMessage(
        "Camera intrinsics recalculated from the matching point pairs.",
      );
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  const saveManual = async () => {
    ...
    if (cameraPoints.length !== scenePoints.length || pairCount < 4) {
      setError(
        `Saving calibration requires equal point counts with at least 4 pairs. Camera: ${cameraPoints.length}, scene: ${scenePoints.length}.`,
      );
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      const translation = Array.isArray(solved.position)
        ? solved.position
        : camera.translation || [0, 0, 0];
      const rotation = Array.isArray(solved.euler)
        ? solved.euler
        : camera.rotation || [0, 0, 0];
      const saved = await apiFetch<Row>(
        `/api/v2/cameras/${encodeURIComponent(idOf(camera))}?revision=${camera.revision}`,
        {
          method: "PUT",
          body: JSON.stringify({
            name: camera.name,
      ...
      await apiFetch(
        `/api/v2/cameras/${encodeURIComponent(idOf(saved))}/runtime-update`,
        {
          method: "POST",
          body: JSON.stringify({
            translation,
  ...
  const waitForRegistration = async () => {
    ...
    let status = await apiFetch<Row>(
      `/api/v2/autocalibration/scenes/${encodeURIComponent(sceneId)}/registration`,
      { method: "POST", body: "{}" },
    );
    for (
      let i = 0;
      i < 90 && ["registering", "busy"].includes(String(status.status));
      i += 1
    ) {
      await new Promise((resolve) => window.setTimeout(resolve, 1000));
      status = await apiFetch<Row>(
        `/api/v2/autocalibration/scenes/${encodeURIComponent(sceneId)}/registration`,
      );
    }
    if (status.status !== "success")
      throw new Error(
        String(status.message || status.status || "Scene registration failed"),
      );
    return status;
  };
  ...
  const autoCalibrate = async () => {
    ...
    try {
      ...
      if (!image)
        throw new Error("Calibration frame did not contain an image.");
      await apiFetch(
        `/api/v2/autocalibration/cameras/${encodeURIComponent(idOf(camera))}/calibration`,
        {
          method: "POST",
          body: JSON.stringify({
            image,
      ...
      for (let i = 0; i < 90; i += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 1000));
        result = await apiFetch<Row>(
          `/api/v2/autocalibration/cameras/${encodeURIComponent(idOf(camera))}/calibration`,
        );
        if (
          !["busy", "calibrating", "not_started"].includes(
      ...
      if (result.status !== "success")
        throw new Error(String(result.message || "Auto-calibration failed"));
      if (Array.isArray(result.calibration_points_2d))
        setCameraPoints(result.calibration_points_2d);
      if (Array.isArray(result.calibration_points_3d))
        setScenePoints(result.calibration_points_3d);
      ...
      setMessage(
        `${scene.camera_calibration || "Auto"} calibration completed and camera pose saved.`,
      );
      onSaved();
    } catch (e) {
      setError(String(e));
  ...
  if (!camera)
    return (
      <div className="empty-state">
        ...
        <p>Add a camera before calibrating this scene.</p>
      </div>
    );
      ...
      <div className="panel calibration-controls">
        <div className="panel-title">
          <div>
            ...
            <p>
              2026.2-compatible point correspondence and auto-calibration
              workflow
            </p>
          </div>
        </div>
        <div className="scene-form-grid calibration-form">
          ...
          <label>
            Strategy
            <input
              value={String(scene.camera_calibration || "Manual")}
              readOnly
            />
          </label>
          ...
          <label className="checkbox-label">
            <input
              type="checkbox"
              checked={lockFx}
              onChange={(e) => setLockFx(e.target.checked)}
            />
            Lock FX
          </label>
          <label className="checkbox-label">
            <input
              type="checkbox"
              checked={lockFy}
              onChange={(e) => setLockFy(e.target.checked)}
            />
            Lock FY
          </label>
        </div>
        <div className="editor-actions">
          ...
          >
            Refresh calibration frame
          </button>
          <button
            className="btn"
            disabled={busy || pairCount < 6}
          ...
          {scene.camera_calibration !== "Manual" && (
            <button
              className="btn"
        ...
        {message && (
          <div className="notice-box calibration-message">{message}</div>
        )}
      ...
      <div className="calibration-panes">
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Camera image</h2>
              <p>Double-click image points in order.</p>
            </div>
            <span className="status-pill">{cameraPoints.length} points</span>
          </div>
          <div className="camera-cal-frame">
            {frame?.image ? (
              <div className="camera-cal-image" onDoubleClick={addCameraPoint}>
                <img
              ...
              <div className="camera-feed-placeholder">
                Refresh the calibration frame to begin.
              </div>
            )}
          </div>
        </section>
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Scene map</h2>
              <p>Double-click the corresponding 3D points in the same order.</p>
            </div>
            <span className="status-pill">{scenePoints.length} points</span>
          </div>
          <ThreeScene
            mapPath={mapPath}
            objects={[]}
      ...
      <div className="workspace-grid calibration-bottom">
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Point pairs</h2>
              <p>Minimum 4; 6+ enables focal-length estimation.</p>
            </div>
            <span
              className={
                cameraPoints.length === scenePoints.length && pairCount >= 4
                  ? "status-pill ok-pill"
                  : "status-pill warning-pill"
              }
            >
              {cameraPoints.length} / {scenePoints.length}
            </span>
          </div>
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {Array.from(
                  { length: Math.max(cameraPoints.length, scenePoints.length) },
                  (_, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td>
                      <td>
        ...
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Auto-calibration context</h2>
              <p>
          ...
          <div className="stack-list">
            {markers.map((marker) => (
              <div key={String(marker.marker_id || marker.uid)}>
                <b>AprilTag {String(marker.apriltag_id ?? marker.marker_id)}</b>
                <span>
                  {Array.isArray(marker.dims)
                    ? marker.dims
            ...
            {!markers.length && (
              <div className="table-empty">
                {scene.camera_calibration === "AprilTag"
                  ? "No registered AprilTag markers yet."
                  : "Calibration markers are used by AprilTag scenes."}
              </div>
            )}
          </div>
        </section>

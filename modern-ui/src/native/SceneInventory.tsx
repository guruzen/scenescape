...
export default function SceneInventory({ isAdmin }: { isAdmin: boolean }) {
  ...
  const save = async () => {
    ...
    try {
      const body: Row = {
        ...
        map_center_lat:
          draft.map_center_lat === null || draft.map_center_lat === ""
            ? null
            : Number(draft.map_center_lat),
        map_center_lng:
          draft.map_center_lng === null || draft.map_center_lng === ""
            ? null
            : Number(draft.map_center_lng),
        ...
        apriltag_size:
          draft.apriltag_size === null || draft.apriltag_size === ""
            ? null
            : Number(draft.apriltag_size),
        number_of_localizations:
          draft.number_of_localizations === null ||
          draft.number_of_localizations === ""
            ? null
            : Number(draft.number_of_localizations),
        ...
        minimum_number_of_matches:
          draft.minimum_number_of_matches === null ||
          draft.minimum_number_of_matches === ""
            ? null
            : Number(draft.minimum_number_of_matches),
        inlier_threshold:
          draft.inlier_threshold === null || draft.inlier_threshold === ""
            ? null
            : Number(draft.inlier_threshold),
      };
      ...
      let saved: Row;
      if (selected) {
        saved = await apiFetch<Row>(
          `/api/v2/scenes/${encodeURIComponent(idOf(selected))}?revision=${selected.revision}`,
          {
            method: "PUT",
        ...
        saved = await apiFetch<Row>("/api/v2/scenes", {
          method: "POST",
          body: JSON.stringify(body),
        });
      }
      ...
      if (mapFile || polycamFile) {
        ...
        saved = await apiFetch<Row>(
          `/api/v2/scenes/${encodeURIComponent(idOf(saved))}/files`,
  ...
  const remove = async () => {
    if (
      !selected ||
      !isAdmin ||
      !window.confirm(`Delete ${nameOf(selected)} and its scene media?`)
    )
      return;
    setBusy(true);
    try {
  ...
  const clearMap = async () => {
    ...
    setBusy(true);
    try {
      const saved = await apiFetch<Row>(
        `/api/v2/scenes/${encodeURIComponent(idOf(selected))}?revision=${selected.revision}`,
        {
          method: "PUT",
  ...
  const exportScene = async () => {
    ...
    try {
      const blob = await apiBlob(
        `/api/v2/scenes/${encodeURIComponent(idOf(selected))}/export`,
      );
  ...
  const generateMesh = async () => {
    ...
    try {
      ...
      const start = await apiFetch<Row>(
        `/api/v2/scenes/${encodeURIComponent(idOf(selected))}/mesh`,
        { method: "POST", body: form },
      );
      const requestId = String(start.request_id);
      for (let attempt = 0; attempt < 150; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 2000));
        const status = await apiFetch<Row>(
          `/api/v2/scenes/${encodeURIComponent(idOf(selected))}/mesh/status?request_id=${encodeURIComponent(requestId)}`,
        );
        setMapping(status);
        if (status.state === "complete") {
          if (
            status.success === false ||
            (!status.finalized && status.result?.success === false)
          ) {
            throw new Error(
              String(
                status.error ||
                  status.result?.error ||
                  "Mapping reconstruction failed",
              ),
            );
          }
          load();
          const refreshed = await apiFetch<Row>(
            `/api/v2/scenes/${encodeURIComponent(idOf(selected))}?revision=${selected.revision}`,
          );
          ...
          setMappingVideo(null);
          setMessage(
            "Generated mesh finalized and scene/camera geometry refreshed.",
          );
          return;
        }
        if (status.state === "failed" || status.success === false) {
          throw new Error(
            String(status.error || "Mapping reconstruction failed"),
          );
        }
        setMessage(`Mapping Service: ${status.state || "processing"}…`);
      }
      throw new Error(
        "Mapping reconstruction did not complete within the polling window.",
      );
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <div className="kicker">Configuration · control plane</div>
          <h1>Sites, floors & scenes</h1>
        </div>
      ...
      <div className="config-grid scene-lifecycle-layout">
        <section className="panel">
          <div className="toolbar scene-toolbar">
            <input
              ...
              placeholder="Search Sites, floors & scenes"
            />
          ...
          <div className="scene-list">
            {filtered.map((row) => (
              <button
                className={
                  selected && idOf(selected) === idOf(row)
                    ? "scene-config-row active"
                    : "scene-config-row"
                }
                key={idOf(row)}
                onClick={() => open(row)}
              >
                <span>
                  <b>{nameOf(row)}</b>
                  <small>{idOf(row)}</small>
                </span>
                <span>
                  <small>
                    {row.map ? "Map configured" : "No map"} ·{" "}
                    {row.map_type || "map_upload"}
                  </small>
                </span>
              </button>
            ))}
        ...
        <section className="panel scene-editor">
          <div className="panel-title">
            <div>
              <h2>{selected ? `Edit ${nameOf(selected)}` : "Create scene"}</h2>
              <p>2026.2 scene lifecycle controls</p>
            </div>
            {selected && (
          ...
          {!isAdmin && !selected ? (
            <div className="table-empty">
              Administrator role required to create scenes.
            </div>
          ) : (
            <>
              <div className="scene-form-grid">
                ...
                <label>
                  Map type
                  <select
                    value={String(draft.map_type || "map_upload")}
                    onChange={(e) => field("map_type", e.target.value)}
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    value={draft.scale ?? ""}
                    onChange={(e) =>
                      field("scale", numberOrNull(e.target.value))
                    }
                  />
                </label>
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    checked={Boolean(draft.use_tracker)}
                    onChange={(e) => field("use_tracker", e.target.checked)}
                  />
                  Use tracker
                </label>
                <label>
                  ...
                  <input
                    type="file"
                    accept=".png,.jpg,.jpeg,.glb,.ply,.zip"
                ...
                <label>
                  ...
                  <input
                    ...
                    accept=".zip"
                    onChange={(e) =>
                      setPolycamFile(e.target.files?.[0] || null)
                    }
                  />
                </label>
              </div>
              {draft.map_type === "geospatial_map" && (
                <div className="scene-subsection">
                  <h3>Geospatial configuration</h3>
                  <div className="scene-form-grid">
                    <label>
                      Provider
                      <select
                        value={String(draft.geospatial_provider || "google")}
                        onChange={(e) =>
                          field("geospatial_provider", e.target.value)
                        }
                      >
                    ...
                    <label>
                      ...
                      <input
                        type="number"
                        step="any"
                        value={draft.map_zoom ?? 15}
                        onChange={(e) =>
                          field("map_zoom", Number(e.target.value))
                        }
                      />
                    </label>
                    <label>
                      ...
                      <input
                        type="number"
                        step="any"
                        value={draft.map_center_lat ?? ""}
                        onChange={(e) =>
                          field("map_center_lat", numberOrNull(e.target.value))
                        }
                      />
                    </label>
                    <label>
                      ...
                      <input
                        type="number"
                        step="any"
                        value={draft.map_center_lng ?? ""}
                        onChange={(e) =>
                          field("map_center_lng", numberOrNull(e.target.value))
                        }
                      />
                    </label>
                    <label>
                      ...
                      <input
                        type="number"
                        step="any"
                        value={draft.map_bearing ?? 0}
                        onChange={(e) =>
                          field("map_bearing", Number(e.target.value))
                        }
                      />
                    </label>
                    <label className="checkbox-label">
                      <input
                        type="checkbox"
                        checked={Boolean(draft.output_lla)}
                        onChange={(e) => field("output_lla", e.target.checked)}
                      />
                      Output LLA
                    </label>
                    <label className="wide">
                      ...
                      <textarea
                        ...
                        placeholder="[[lat,lng,alt], ... four corners]"
                      />
                    </label>
                  </div>
                  <GeospatialMap
                    ...
                    provider={
                      draft.geospatial_provider === "mapbox"
                        ? "mapbox"
                        : "google"
                    }
                    settings={{
                    ...
                    onGenerated={(value) => {
                      ...
                      setMessage(
                        "Geospatial map generated. Save scene to persist the PNG and LLA bounds.",
                      );
                    }}
                  />
                </div>
              ...
              <div className="scene-subsection">
                <h3>Advanced tracking & calibration</h3>
                <div className="scene-form-grid">
                  <label>
                    Regulate rate (Hz)
                    <input
                      ...
                      min="0.001"
                      step="any"
                      value={draft.regulated_rate ?? 30}
                      onChange={(e) =>
                        field("regulated_rate", numberOrNull(e.target.value))
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      min="0.001"
                      step="any"
                      value={draft.external_update_rate ?? 30}
                      onChange={(e) =>
                        field(
                  ...
                  <label>
                    Calibration type
                    <select
                      value={String(draft.camera_calibration || "Manual")}
                      onChange={(e) =>
                        field("camera_calibration", e.target.value)
                      }
                    >
                  ...
                  <label>
                    ...
                    <input
                      type="number"
                      step="any"
                      value={draft.apriltag_size ?? ""}
                      onChange={(e) =>
                        field("apriltag_size", numberOrNull(e.target.value))
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="number"
                      step="1"
                      value={draft.number_of_localizations ?? ""}
                      onChange={(e) =>
                        field(
                          "number_of_localizations",
                          numberOrNull(e.target.value),
                  ...
                  <label>
                    ...
                    <input
                      value={String(draft.global_feature ?? "")}
                      onChange={(e) => field("global_feature", e.target.value)}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      type="number"
                      step="1"
                      value={draft.minimum_number_of_matches ?? ""}
                      onChange={(e) =>
                        field(
                          "minimum_number_of_matches",
                          numberOrNull(e.target.value),
                  ...
                  <label>
                    ...
                    <input
                      ...
                      min="0"
                      step="any"
                      value={draft.inlier_threshold ?? ""}
                      onChange={(e) =>
                        field("inlier_threshold", numberOrNull(e.target.value))
                      }
                    />
                  </label>
                  <label className="wide">
                ...
                <div className="scene-form-grid">
                  ...
                  <label className="wide">
                    ...
                    <textarea
                      ...
                      placeholder="4x4 JSON matrix or null"
                    />
                  </label>
                </div>
              </div>
              {selected && (
                <div className="scene-subsection">
                  ...
                  <div className="media-summary">
                    <code>{String(selected.map || "No map")}</code>
                    <code>{String(selected.thumbnail || "No thumbnail")}</code>
                  </div>
                </div>
              )}
              <div className="editor-actions scene-editor-actions">
                {selected && (
                  <button className="btn" onClick={() => void exportScene()}>
                    Export ZIP
                  </button>
                )}
                {selected && (
                  <button className="btn" onClick={() => void checkMapping()}>
                    Mapping status
                  </button>
                )}
                {selected && (
                  <label className="btn">
                    Optional mapping video
                    <input
                      className="hidden-file"
                      type="file"
                      accept=".mp4,.mov,.mkv,.webm,.avi"
                      onChange={(e) =>
                        setMappingVideo(e.target.files?.[0] || null)
                      }
                    />
                  </label>
                )}
                {selected && mappingVideo && (
                  <span className="muted">{mappingVideo.name}</span>
                )}
                {selected && (
                  <button
                    className="btn"
                ...
                {isAdmin && (
                  <button
                    ...
                    onClick={() => void save()}
                  >
                    {busy ? "Working…" : "Save scene"}
                  </button>
                )}
              </div>
              {mapping && (
                <pre className="mapping-status">
